...
const updateFile = () => {
  fs.writeFileSync(
    "./modules/Reservas.json",
    JSON.stringify(reservas, null, "\t"),
    (error) => {
      if (error) {
...
export function findWithFilters(query) {
  ...
  return reservas.filter(
    (r) =>
      (r.branchId == branchId || !branchId) &&
      (new Date(r.dateTime).toLocaleDateString() == dateTime || !dateTime) &&
...
export function findById(id) {
  ...
  else throw "No existe reserva con ese id"
}
